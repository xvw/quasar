...
let () =
  test "Test for to_array" (fun () ->
      A.to_array id $ arr "[1, 2, 3, 4]" == [| 1; 2; 3; 4 |];
      A.to_array id $ arr "[]" == [||];
...
let () =
  test "Test for to_list" (fun () ->
      A.to_list id $ arr "[1, 2, 3, 4]" == [ 1; 2; 3; 4 ];
      A.to_list id $ arr "[]" == [];
      A.to_list (fun x -> x * 2)
      $ arr "[1, 2, 3, 4]" == [ 2; 4; 6; 8 ])
;;
...
let () =
  test "Test for map" (fun () ->
      A.map id $ A.empty () == arr "[]";
      A.map id $ A.from_list id [ 1; 2; 3; 4 ] == arr "[1, 2, 3, 4]";
      A.map succ
